    ...
    <div className="max-w-5xl mx-auto py-6 px-4 ">
      <Card className="shadow-lg border-2 overflow-hidden ">
        ...
        <CardContent className="p-6">
            {user.skills && user.skills.length>0 ? <div className="flex flex-wrap gap-3">{user.skills.map((skill,idx)=> <div className="border-2 group relative inline-flex border-gray-400/40 py-2 items-center shadow-md hover:shadow-red-300  duration-200 transition-all gap-4 px-4 rounded-2xl pl-4 pr-3" key={skill}>{<span>{skill}</span>}{ isYourAccount && <button disabled={btnloading}  className="cursor-pointer text-red-500" onClick={(e)=> removeSkill(skill)} ><X size={14}/></button>} </div>)}</div> :<div></div>}
        </CardContent>
      </Card>
    </div>
  )
