...
const applyForJob = AsyncHandler(async (req, res, next) => {
    ...
    if (user.role !== 'jobseeker')
        throw new ApiError(403, "Only Jobseekers can apply for jobs");
    const applicantId = user.user_id;
    const resume = user.resume;
    if (!resume)
        throw new ApiError(400, "Resume Not uploaded , Please upload your resume first");
    ...
    if (!job || !job.is_active)
        throw new ApiError(404, "Job Not more active");
    const now = Date.now();
...
const getAllApplications = AsyncHandler(async (req, res, next) => {
    ...
    if (user.role === 'recruiter')
        throw new ApiError(403, "Recruiter can't apply for Jobs");
